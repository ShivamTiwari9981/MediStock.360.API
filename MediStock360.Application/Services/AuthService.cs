    ...
    public class AuthService : BaseService, IAuthService
    {
        ...
        public async Task<ApiResponse<LoginResponseDto>> LoginAsync(LoginRequestDto dto)
        {
            try
            {
                ...
                var clientsList = new List<UserClientDto>
                {
                    MapToUserClientDto(client)
                };
                var activeClientDto = MapToUserClientDto(client);

                // 4. Resolve Store(s) for the Client
                ...
                bool isOnboardingCompleted = client.IsOnboardingCompleted;
                int onboardingStep = client.OnboardingStep;
        ...
        private string GenerateJwtToken(User user, Client? client, Store? activeStore, List<Role> roles, List<string> permissionCodes, DateTime expiresAt, bool isOnboardingCompleted, int onboardingStep)
        {
            ...
            var claims = new List<Claim>
            {
                ...
                new Claim(Global.Claim_Types.IsCompanyProfileCreated, isOnboardingCompleted.ToString()),
                new Claim("IsOnboardingCompleted", isOnboardingCompleted.ToString()),
                new Claim("OnboardingStep", onboardingStep.ToString()),
